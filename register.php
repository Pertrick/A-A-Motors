...
$error ='';
	if(isset($_SESSION['login'])){
		...
		if(!empty($_REQUEST['firstname']) && !empty($_REQUEST['lastname']) && !empty($_REQUEST['email']) && !empty($_REQUEST['password'])){
			extract($_REQUEST);
			$register = $user->registerUser($firstname,$lastname,$email,$password);
			if ($register) {
			// Registration Success
			$_SESSION['success'] = '<div class="color:green; text-align:center;">Registration successful</div>';
...
<html lang="en">
<head>
	<title>A&A Motors | Register</title>
	<meta charset="UTF-8">
